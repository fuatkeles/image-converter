...
function App() {
  ...
  const handleConvert = async (index) => {
    ...
    try {
      const response = await axios.post('https://image-converter-rb0g.onrender.com/convert', formData, {
        responseType: 'blob',
      });
  ...
  const handleAddGeotag = async (index) => {
    ...
    try {
      const response = await axios.post('https://image-converter-rb0g.onrender.com/add-geotag', formData, {
        responseType: 'blob',
      });
      const url = URL.createObjectURL(response.data);
